...
After the user's explicit resumption, the independent report is complete. The next comparisons are a separately reviewed Hold/Resume repair and an isolated tail-truck recorded-route guide under the original graph. Keep command and movement changes separately attributable. Normal keyboard/panel use, supply transfer, multiplayer, varied routes, unpaved coverage and public integration remain open.
