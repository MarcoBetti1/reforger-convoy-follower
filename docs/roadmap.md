...
**Latest reconciliation:** preserve mod HEAD `82f4b3f`, independent harness HEAD `419dba5`, and every newer local source/evidence artifact. The private two-follower rear-pacing comparison (`BA45A5BA...`) remains physical/strict FAIL: peak gaps **81.7383 / 101.084 m**, despite **180.626 seconds** of zero hold drift and retained identities/captured Waits. Its final report, natural client exit and recorder closure are now recorded; nine runtime and 64 shutdown errors remain. The private rear-pacing enable is already back at zero. Preserve the preceding `D4E3874C...` comparison separately: **52.1781 / 70.0129 m**, 180.676 seconds of zero drift, and physical/strict FAIL. Neither experiment is promoted production behavior.

Callback-action v3/v4/v5 remain failed under their original contracts. V6 (`5EA78318...`) subsequently passed review, all five configurations, packaging and its declared engine-action calibration: 453 writes/readbacks, 405 powered intervals / 20.062 m, 20.0071 m signed uphill travel, and zero unexpected gaps. One prospectively qualified neutral-baseline sleep remains counted without motion credit. Natural closure retained nine runtime and 64 shutdown errors, so strict full-run status is FAIL. This does not establish normal desktop input or precise stopping. The matching one-follower captured-Wait control is finalized: physical/strict FAIL at 83.192 m peak gap, 180.18 seconds with zero hold drift, and natural closure retaining nine runtime and 64 shutdown errors. The first trail-guide adapter subsequently compiled and ran as frozen `CE3DEF31...`: genuine route acquisition, but spacing and progress-guard failure, 267.129 m peak and no formal hold. Its failed comparison is preserved. The diagnostic-only v3 successor passed validation and packaging, then failed its live repeat at 274.773 m peak after native reversal and OFF_ROUTE; no formal hold occurred. A separate original-graph comparison is next. See [the experiment record](convoy-trail-guide-experiment.md) and current validation for the exact frozen package and next comparison. See [current validation](convoy-validation-current.md) and [input calibration](convoy-input-calibration.md) for exact scope; do not restart v6 from the older source-only label.

Preserve the earlier `E01A1EC3...` one-follower **physical PASS / strict FAIL** separately: 52.0785 m peak, 180.313 seconds / 180 samples per truck, zero measured hold drift, and four confirmed reverse episodes. Nine runtime errors, an early native-close callback at 29999.8 ms, forced termination and unobserved shutdown prevent a full-run pass. The later v5 action-input fixture also exercised its single early-callback deferral before natural exit; this does not verify the separate paced probe's corresponding branch. None of these private candidates is promoted production behavior; copied-tree redistribution, ordinary human input, repeatability, broader driving, multiplayer and the full supply journey remain open.
