...
**Later original-graph evidence:** the one-follower comparison has a finalized physical PASS / strict full-run FAIL. The two-follower independent analysis is also finalized: physical/strict FAIL, with 46.0147 / 108.378 m link peaks and a 180.312-second zero-drift hold, followed by normal closure. Preserve both runs. The private command repair, Hold/Resume fixture and parked-lead policy subsequently compiled and ran in `FA913F7A...`: baseline 52.6805 m peak gap and 180.364-second zero-drift hold, followed by physical explicit-Hold failure before Resume. Natural closure and nine runtime / 62 shutdown errors remain. The tail-guide comparison is still cache-only, reviewed but uncompiled and unrun. See [current validation](convoy-validation-current.md) for this newer command result and [the experiment record](convoy-original-follow-experiment.md) for preceding comparisons; do not restart completed runs because older paragraphs describe work in progress.

**Newest evidence preserved during reconciliation:** keep mod HEAD `506d631`, its earlier `4f56f64` and `3737f08` history, harness HEAD `0d960a0` (after `419dba5`), and all subsequent local work. The newer private command v2 pack `59F79906...` passes the bounded baseline and explicit Hold but fails continuous Resume at an arrival-to-following activity replacement. Its final independent report confirms natural process closure and recording, with nine runtime and 64 shutdown errors. Preserve the integrated private Hold controller, separate unrun tail-guide candidate and cache-only authored-export draft. Use [current validation](convoy-validation-current.md) for exact provenance and the next queued decision; no private branch is production-promoted.

Earlier rear-pacing `BA45A5BA...` remains physical/strict FAIL. V6 on `5EA78318...` completed only its narrow engine-action calibration; ordinary input is unresolved. The same pack's captured-Wait control failed spacing at 83.192 m despite 180.18 seconds of zero-drift hold. Trail-guide `CE3DEF31...` failed at 267.129 m peak gap, and `F134EAA1...` failed at 274.773 m with native reversal before OFF_ROUTE and no formal hold. Preserve these completed comparisons, errors, packages and source snapshots. No implementation or test is started by this documentation reconciliation, and nothing here changes the historical checkpoint below or authorizes execution during setup.
