...
## Next comparison under explicit continuation

Keep this option off by default. The saved `.cache/native-waypoint-queue-analysis.md` already records installed completion/cancellation behavior and ownership concerns: native completion can cancel movement activity, and untracked queue tails would escape the controller's single-waypoint cleanup. A queue alone is not established as a continuity fix. Its proposed alternative is a test-only persistent native predecessor-follow activity with vehicle planning explicitly enabled, compared with ordinary MOVE. That proposal is physically unvalidated and could still replan, stop or cut corners; it is not proof of driven-route following.

Use the smallest discriminating comparison from that research under the explicit continuation. Preserve the original predecessor chain, useful navigable targets, one cruise writer, possession/command cleanup, unchanged 60 m gates and full 180-second hold observation. Log activity, target and waypoint identities, actual paths and completion/power timing. If testing a queued alternative, own and cancel every queued entry. Do not combine the first continuity comparison with pacing changes or suppress native avoidance based on the failed retention result.
