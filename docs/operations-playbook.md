...
| --- | --- |
| Game | Stable 1.8.0.13, installed in a local Steam library; the launcher resolves its executable |
| Workbench | Installed as Arma Reforger Tools in the same Steam library; the helper resolves its executable |
| Client Workshop cache | `%USERPROFILE%\Documents\My Games\ArmaReforger\addons` |
| Client logs | `%USERPROFILE%\Documents\My Games\ArmaReforger\logs` |
| Dedicated server | Installed in a separate local test-tools directory; version 1.8.0.13 matches the client |
| SteamCMD | Installed in that test-tools directory; the setup command records its exact executable path locally |
...
The current `workbench:open` World Editor command includes `-run` as well as `-load`, but a September 25 follow-up showed `AUTO_INIT` during World Editor entity initialization **before** gameplay. Reacquire the editor window and press F5 once to start the preview. Require a later `GAME` transition and `AUTO_READY` or equivalent gameplay marker, then inspect the live viewport. Do not treat an editor-time `AUTO_INIT` as a completed gameplay launch; the strict parser deliberately selects the F5 session.

In the isolated road-corridor survey, an injected F5 did not start the preview after the editor loaded. A click on the green **Play** toolbar control in the freshly observed World Editor did start it, confirmed by a later `OnGameStateChanged = GAME` log and the survey's terminal marker. Reacquire the window and toolbar position for each launch rather than reusing the observed `(344, 77)` screen coordinate.
...
The later test-only AI pilot did drive the lead M923 using a normal waypoint. In the September 25 F5 sustained run it covered 313 m while its single follower covered 237 m and closed to a 55 m gap, yielding `AUTO_RESULT: PASS sustained road following`. The local camera hook had to close Game Master and select the player camera; merely moving the Game Master camera object left the editor HUD and distant harbor view on screen. The first explicit unload release was accepted but moved 0 m. A one-shot fallback later cleared the bay, and an OpenRoad one-truck run reached `UNLOAD_RETURN_PARKED`: the driver stayed seated in a settled waiting slot 18.17 m from release. A two-truck scripted F5 run confirmed Unit 1 parking in the return slot, Unit 2 pulling up to the unload bay, and both drivers merging into the return roster. Their physical homeward following and the player-facing rear menu still need a successful live test. The harbor-gate two-truck run stalled Unit 1 near the base exit, so separate `Worlds/Tests/ConvoyFollower_Arland_OpenRoad_Auto_1Truck.ent` through `3Trucks.ent` worlds now stage trucks directly on the connected hill road. Use those for the cleaner 1/2/3 baseline while retaining the harbor worlds as a gate stress case. Window-title `ffmpeg gdigrab` captured a stale loading frame while gameplay visibly updated; `ffmpeg ddagrab` captured 15 distinct live frames in a three-second monitor test. Use `npm run convoy:record -- --backend ddagrab --output-idx 1 ...` for a whole-monitor demo capture, and inspect the MP4 before claiming video proof.

### Vanilla GM input delivery control (September 26)

The fresh `vanilla-ui-delivery-v1` run loaded only `core` and base `ArmaReforger` on game 1.8.0.13, directly into `worlds/GameMaster/GM_Arland.ent`. It reused only the preceding panel run's engine/game settings and requested window `1280 × 720` at `(640,680)` with `-forceUpdate`. Escape and a click on native **Edit scenario properties** had no observed effect; the pause toolbar only hovered while displayed simulation time continued advancing. This reproduces a UI-delivery symptom without Convoy Follower. It was the **native GM toolbar, not an opened MapMenu**, and establishes neither the cause nor restored player driving. Keep hover, successful tool return, OS close, raw input and native action delivery distinct. Exact settings hashes, UTC inputs and log evidence are in `.cache/client/runs/vanilla-ui-delivery-v1/result.md`.

Alt+F4 at 09:20:10.862 UTC closed the client normally; `Game destroyed` followed at 09:20:11.441. The full console retains **nine startup/runtime and 62 shutdown errors**, so normal closure is not clean lifecycle acceptance. The recorder's plain-pipe stdin rejected `q`; its existing 360-second limit later finalized 5,399 frames with exit 0. The private full-monitor recording includes a substantial desktop tail after game exit. Use PTY stdin when an early recorder stop is needed, keep originals private, and do not count the tail as gameplay or change OS/security settings based on this control.

### Convoy panel owner-pilot preview (September 25)

**September 26 packed follow-up:** `.cache/client/runs/panel-input-special-key-v2/` used the compiled ButtonSlot label correction. Labels rendered centered/readable and the full log contains no FrameSlot/ButtonSlot errors; it still retains 11 runtime errors (nine baseline signatures and two native map layout/mouse-mask errors). Requested window content position `(640,680)` matched observed outer bounds `(639,649)`, `1282 × 752`. Unfocused `-forceUpdate` timing remains unverified.

Initial game activation failed at 08:39:08.174 and 08:39:18.786 UTC. Stop uncertain input after such failures. Later, activating an existing neutral Notepad window at 08:41:20.710, then freshly selecting/activating the game at 08:41:31.975 succeeded; Notepad content was not edited. This recovered tool focus once, without proving a general workaround. Actual Hold clicks at 08:41:47.907–47.990 and 08:42:13.162 then produced no order marker or feedback. Close Map at 08:42:30.197 highlighted the button but left the map open. Hover/highlight and successful tool return do not prove a click reached the handler. No planned Up key was delivered. OSK remained elevated; its causal role is unproved.

The 300-second launcher fallback ended this run. Alt+F4 attempted near the deadline failed with `foreground window did not report a process id`; no normal-close claim follows from process exit zero. The log has no cleanup or Game destroyed marker. Recorder quit normally at 4,697 frames (about 313.133 seconds). Preserve the result and exact external-input record. The separate candidate adds at most 12 widget down/up/click traces and resolves only buttons belonging to the live convoy panel; it requires a new frozen comparison before being called a fix. The September 25 successful mouse interaction below remains historical evidence on its earlier layout/build.

Open `Worlds/Tests/ConvoyFollower_Arland_OpenRoad_PanelPilot.ent` with `--authorize-local-test-scripts`, wait for World Editor startup to finish, then press F5. The test-only fixture seats the local owner in the lead truck, recruits one follower, and opens the native MapMenu through its supported menu manager. This fixture-opened map is **not** evidence that an injected or physical **M** key opens it in normal play. In the frozen-source run, the right-side convoy panel was visually readable and showed Unit 1, its assigned truck, position, actual state, and follow target. Clicking **Hold all seated** produced a server acceptance log, then the roster changed to `holding in vehicle` and status to `completed: all trucks holding in vehicles`. Selecting Unit 1 highlighted its row. **Resume all** restored `following` and showed `completed: convoy following`. **Close Map** returned to the third-person driver view; clicking the viewport changed the camera angle, confirming mouse camera input after closing. A synthetic **W** tap did not move the vehicle, and a synthetic **M** tap did not reopen the map; neither proves a production input bug or sustained driving control. Evidence: `.cache/workbench-gui-runs/2026-09-25T16-37-32-174Z-20256/console.log`, `.cache/test-videos/panel-pilot-dpi-v3.png`, and `.cache/test-videos/panel-pilot-dpi-v3.mp4`.

The bundled Sky controller sometimes reports `window crop is outside captured monitor` for a restored Workbench window. On this machine, select the unique returned World Editor window, send `Alt+Space` then `x` through Sky to maximize it, reobserve the same window, and only then use screenshot-relative clicks. Its accessibility tree changed the title-bar control from **Maximize** to **Restore**, and full screenshots worked afterward. Keep addon source and test worlds frozen while F5 runs: a new test script was saved at 11:34:13, Workbench hot-reloaded during the PanelPilot preview at 11:34:15, and the engine raised `Resources are leaking!` at 11:34:17. The timing is consistent with the source change disrupting the preview; the assertion itself did not identify the cause. A fresh validated pack and GUI process completed the panel test. For current 2560 × 1440 monitor captures, `ddagrab --output-idx 0` captured live Workbench; check the output index on every monitor setup rather than assuming the earlier index 1 still applies.

For the Arland road-width survey, expand the survey layer in World Editor Hierarchy, select `CF_RoadCandidate28` or `CF_RoadCandidate43`, and press **F** to frame it. Framing a small marker placed at road height can put the camera inside the marker or below terrain. Open the viewport **Camera** options, raise **Camera Position Y** to about `120`, set **Camera Angles X** to about `-80`, then close the options to inspect the road from above. Candidate 28 at `(1392, 37, 3036)` appeared relatively straight with trees close to both shoulders; candidate 43 at `(1666, 36, 3237)` curved beside large shoulder boulders and shrubs. Editor screenshots are `.cache/test-videos/road-candidate28-editor.jpg` and `.cache/test-videos/road-candidate43-editor.jpg`. The view checks geometry only; a parked M923 passing another vehicle remains a live-test requirement.

For a full-convoy F5 recording, open `Worlds/Tests/ConvoyFollower_Arland_OpenRoad_Auto_3Trucks.ent`. The test-only [`CF_ConvoyFilmCameraComponent.c`](../addons/ConvoyFollower/Scripts/Game/Tests/CF_ConvoyFilmCameraComponent.c) is attached to `CF_SmokeLead` in that scene only. The smoke probe still possesses a player character in the AI-driven lead truck and closes Game Master. After the local player camera exists, the film component finds the last follower, calculates a roadside elevated view of the lead-to-tail span, spawns a separate `CameraBase` entity, selects it with `CameraManager.SetCamera`, and updates that camera's world transform and `ApplyTransform` each post-frame. The separate selected camera is essential: the earlier `BaseWorld.SetCameraEx` call on the active `PlayerCamera` logged its new position but rendered the ordinary chase view. The selected `CameraBase` method passed Workbench validation and a live screenshot showed all four trucks in one road view. Its `AUTO_FILM_CAMERA_SETUP` and `AUTO_FILM_CAMERA` log lines confirm selection and tracking; inspect recorded frames as the visual check. The technique follows [Bohemia's camera manager contract](https://community.bistudio.com/wikidata/external-data/arma-reforger/ArmaReforgerScriptAPIPublic/interfaceCameraManager.html), which requires the selected camera to be registered.

After `convoy:record` finishes and the MP4 closes, make a compact gameplay-only review copy with `npm run convoy:clip -- --input .cache/test-videos/raw.mp4 --output .cache/test-videos/review.mp4 --start-seconds 30 --duration-seconds 45 --execute`. Omit the trim arguments for the full run. The default crop is `2060:1060:1000:120` (width, height, left, top) measured from the 3440×1440 monitor's World Editor gameplay viewport; override `--left`, `--top`, `--width`, or `--height` if the window moved. The helper preserves the source, fits the crop into 1920×1080 with narrow black bars rather than stretching it, and keeps any existing audio track. A three-second sample from a completed two-truck recording rendered at 1920×1080 and visually contained only gameplay. Do not run ffprobe or clipping against an MP4 still being written: it lacks its final `moov` index until the recorder exits. This ffmpeg build has no WASAPI input device, and DirectShow lists microphones but no system loopback or Stereo Mix, so the current `ddagrab` recorder does not include game audio.
...
```powershell
$gameExe = '<path-to-ArmaReforgerSteam.exe>'
$workbenchExe = '<path-to-ArmaReforgerWorkbenchSteamDiag.exe>'
$baseAddonsDir = Join-Path (Split-Path $gameExe -Parent) 'addons'
...
& $workbenchExe -gproj 'C:\Path\To\addon.gproj' -addonsDir $baseAddonsDir -wbModule=ScriptEditor -validate PC -wbSilent -logsDir 'C:\ReforgerTestLogs'

# Pack an addon after validation succeeds.
& $workbenchExe -gproj 'C:\Path\To\addon.gproj' -addonsDir $baseAddonsDir -wbModule=ResourceManager -packAddon -packAddonDir 'C:\Path\To\packed'
```
...
A bounded run of this combination using an earlier private server-cache profile logged Raven in `Loaded addons`, loaded the requested world, reached `GAME`, and showed usable Game Master controls; the tutorial **Hide** button responded to a click. The new fetcher's cache has the same addon layout and was separately verified. Evidence: `.cache/client/runs/2026-09-24T18-09-19-500Z-29480/logs/console.log`. This does not test Raven's supply commands or delivery behavior.
...
Vanilla control evidence was retained under the private test-tools directory in `vanilla-control-20260924` and `vanilla-public-local-20260924`, each with server and client console logs. All processes were stopped afterward; the default server port had no remaining listener. An in-game Direct Connect attempt could not reach the menu because of the splash/focus issue described above, so only automatic join has a connection result.
...
`client:run --execute` and `convoy:record --execute` refuse to start below **2 GiB available space** on an output filesystem, or when the space probe fails. The read-only check resolves the nearest existing output ancestors through directory junctions before querying the actual filesystem, covering logs, profile/addon-download destinations and video output. It runs before directory creation or process launch. Existing recording output entries, including dangling links, are rejected. Dry runs remain available without a storage probe.

Focused tests and TypeScript checks passed in both repositories. This is a starting-space gate, not a reservation or a mid-run monitor; leave additional headroom for recording duration, downloads and concurrent writers. Recorder session `85625` was already running before the final dangling-link guard and does not live-test that correction.

On September 26, root migrated **279 private evidence files, 30,327,459,293 bytes (30.327 GB)** from `.cache/test-videos` to `D:/ReforgerEvidence/test-videos-2026-09-26`. Every file's SHA-256 and length matched before the originals were removed. The original `.cache/test-videos` path now resolves through a directory junction, preserving existing evidence links. The private migration manifest is `D:/ReforgerEvidence/test-videos-2026-09-26-manifest.json`; keep that manifest and the recordings out of public Git. This was storage recovery, not new gameplay evidence.

### Normal recorder close
